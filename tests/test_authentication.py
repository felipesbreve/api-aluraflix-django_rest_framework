...
class AuthenticationUserTestCase(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user('felipebreve', password='123456')

    def test_auth_user_credenciais_corretas(self):
        """Teste que verifica a autenticação de um user com as credenciais corretas"""
        user = authenticate(username='felipebreve', password='123456')
    ...
    def test_auth_username_incorreto(self):
        ...
        user = authenticate(username='felipebrev', password='123456')
        self.assertFalse((user is not None) and user.is_authenticated)

    def test_auth_senha_incorreta(self):
        """Teste que verifica a autenticação de um user com a senha incorreta"""
        user = authenticate(username='felipebreve', password='123452')
        self.assertFalse((user is not None) and user.is_authenticated)
